...
export async function queryStoredHandlePermission(): Promise<'granted' | 'prompt' | 'denied' | 'none'> {
  try {
...
export async function openAndLoadFile(): Promise<string | null> {
  ...
  try {
    const [handle] = await window.showOpenFilePicker({
      multiple: false,
      types: [{ description: 'TaskFlow data', accept: { 'application/json': ['.json'] } }],
    })
...
export async function saveToFile(data: string, requireUserGesture = false): Promise<void> {
  try {
    const handle = await idbGet<FileSystemFileHandle>(HANDLE_KEY)
    if (handle) {
      const perm = await handle.queryPermission({ mode: 'readwrite' })
